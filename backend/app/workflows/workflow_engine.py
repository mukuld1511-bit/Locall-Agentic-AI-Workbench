...
class WorkflowEngine:
    ...
    def execute_workflow(
        ...
        report_progress(
            "PLANNING",
            "500M Organizer decomposing task and assigning specialists..."
        )
        ...
        try:
            ...
            # Backward-compatible fallback for the actual OrganizerService
            # signature if it does not accept user_id / role.
            try:
                decomposition = self.organizer.plan_task(
                    task_description,
                    user_id,
                ...
                state.status = "FAILED"
                state.error_message = (
                    f"Organizer planning failed: {exc}"
                )
                AUDIT.log_event(
                    user_id=user_id,
                    action="workflow_planning_failed",
                    resource=run_id,
                    details={
                        "error": str(exc),
        ...
        latencies["organizer_latency_ms"] = round(
            (time.time() - org_start) * 1000.0,
            2,
        )

        # Normalize Organizer output.
        if decomposition is None:
            state.status = "FAILED"
            state.error_message = (
                "Organizer returned no workflow decomposition."
            )
            return state

        if isinstance(decomposition, dict):
            raw_steps = decomposition.get("steps", [])
        else:
            raw_steps = getattr(
                decomposition,
                "steps",
                decomposition if isinstance(decomposition, list) else [],
            )

        if raw_steps is None:
            raw_steps = []

        # Never allow an empty/invalid plan to become COMPLETED.
        if not isinstance(raw_steps, list) or len(raw_steps) == 0:
            state.status = "FAILED"
            state.error_message = (
                "Organizer returned an empty workflow plan."
            )
            AUDIT.log_event(
                user_id=user_id,
                action="workflow_planning_failed",
        ...
        for idx, raw_step in enumerate(raw_steps, start=1):
            ...
            if isinstance(raw_step, WorkflowStepPlan):
                step = raw_step

            elif isinstance(raw_step, dict):

                worker_type = (
                    raw_step.get("worker_type")
                    or raw_step.get("worker")
                    or "general"
                )
                ...
                action_name = (
                    raw_step.get("action_name")
                    or raw_step.get("action")
                    or f"Workflow step {idx}"
                )
                ...
                state.status = "FAILED"
                state.error_message = (
                    f"Invalid Organizer workflow step at index {idx}."
                )
                return state

            normalized_steps.append(step)

        # Replace the decomposition steps with normalized executable steps.
        try:
            decomposition["steps"] = normalized_steps
        except Exception:
            # Object-style decomposition.
            setattr(
                decomposition,
                "steps",
                normalized_steps,
        ...
        for step in decomposition.steps:
            ...
            latencies["worker_inference_latency_ms"] = round(
                latencies["worker_inference_latency_ms"]
                + (time.time() - infer_start) * 1000.0,
                2,
            )

            # WorkerInferenceResponse is the canonical response type.
            worker_status = getattr(
                worker_res,
                "status",
                "failed",
            )

            worker_content = getattr(
                worker_res,
                "content",
                "",
            )
            ...
            step_output["worker_response"] = worker_content
            step_output["structured_data"] = getattr(
                worker_res,
                "structured_data",
                None,
            )

            # A specialist failure must fail the workflow step.
            # Never mark a failed inference as completed.
            if str(worker_status).lower() != "success":
                state.failed_steps.append(step.step_number)
                ...
                self._record_step(
                    ...
                    "FAILED",
                    step_output,
                    (time.time() - start_step_time) * 1000.0,
                    "FAIL",
                )
                ...
                state.error_message = (
                    worker_error
                    or f"Worker '{step.worker_type}' failed"
                )
